...
(function () {
  ...
  function normalizedCssText(cssText) {
    return String(cssText || "").toLowerCase().replace(/\s+/g, "");
  }
  ...
  function isSelSupInlineFormattingElement(element) {
    ...
    return !!(getColorKeyFromElement(element) || getWeightKeyFromElement(element));
  }
  ...
  const wrappers = [
    ...
    },
    {
      ...
      defaultBody: "Здесь можно разместить ответ, который будет скрыт до раскрытия блока.",
    },
    {
      label: "Hidden block",
  ...
  // CHANGE 3.9: порядок сворачивания разделов Gutenberg-toolbar.
  const GUTENBERG_COLLAPSE_ORDER = ["tools", "weight", "color", "lines", "quick", "blocks"];
  ...
  function rememberTextInput(element) {
    ...
    debugLog("remember textarea", element.id || element.name || element.className);
  }
  ...
  function addCssToDocument(doc) {
    if (!doc || !doc.head || doc.getElementById("selsup-wrapper-editor-css")) return;
  ...
  function addAdminCss() {
    if (!document.head || document.getElementById("selsup-wrapper-admin-css")) return;
  ...
  function createInlineSpan(doc, text, state) {
    ...
    if (state.colorKey) {
      ...
      span.classList.add(inlineColorClassMap[state.colorKey] || "selsup-text-color");
    }

    if (state.weightKey) {
      span.setAttribute("data-selsup-weight", state.weightKey);
      span.classList.add(inlineWeightClassMap[state.weightKey] || "selsup-text-weight");
    }
  ...
  function normalizeSelSupInlineDom(root) {
    ...
    Array.from(root.querySelectorAll('span[data-selsup-normalized="1"]')).forEach(function (span) {
  ...
  function sameSelSupSpan(a, b) {
    ...
    return (
      a.getAttribute("data-selsup-color") === b.getAttribute("data-selsup-color") &&
      a.getAttribute("data-selsup-weight") === b.getAttribute("data-selsup-weight") &&
  ...
  function mergeAdjacentSelSupSpans(root) {
    ...
    while (changed) {
      ...
      Array.from(root.querySelectorAll("span[data-selsup-color], span[data-selsup-weight]")).forEach(function (span) {
  ...
  function getInlineClassName(wrapper) {
    if (wrapper.kind === "inline-color") return inlineColorClassMap[wrapper.colorKey];
    if (wrapper.kind === "inline-weight") return inlineWeightClassMap[wrapper.weightKey];
  ...
  function isSelSupSpanOpenTag(openTag) {
    ...
    return (
      ...
      /class=["'][^"']*(selsup-text-color-|selsup-text-weight-|ss-text-info|ss-text-warning|ss-text-success|ss-text-danger)/i.test(tag) ||
    ...
    function walk(node) {
      Array.from(node.childNodes).forEach(function (child) {
        ...
        if (child.nodeType === 3) {
          ...
          const nextState = insideSelection ? applyWrapperToState(currentState, wrapper) : currentState;
  ...
  function applyInlineStyleToHtmlRange(html, relStart, relEnd, wrapper) {
    const markerId = "selsup-range-" + Date.now() + "-" + Math.floor(Math.random() * 100000);
    const startMarker = '<span data-selsup-range-marker="' + markerId + '" data-selsup-range-start="1"></span>';
    const endMarker = '<span data-selsup-range-marker="' + markerId + '" data-selsup-range-end="1"></span>';
    const doc = document.implementation.createHTMLDocument("");
    const container = doc.createElement("div");
  ...
  function tryApplyInlineStyleInTextareaContext(element, wrapper, start, end, selected) {
    ...
    element.focus();
    element.setRangeText(replacement, range.openStart, range.closeEnd, "select");
  ...
  function insertIntoTextareaWithUndo(element, text, selectStart, selectEnd) {
    ...
    try {
      if (doc.activeElement === element && doc.queryCommandSupported?.("insertText")) {
  ...
  function wrapInTextarea(element, wrapper) {
    ...
    const replacement = wrapper.insertOnly ? parts.open : parts.open + selected + parts.close;
    const innerStart = wrapper.insertOnly ? start + replacement.length : start + parts.open.length;
    const innerEnd = wrapper.insertOnly ? start + replacement.length : start + parts.open.length + selected.length;
    ...
    if (insertIntoTextareaWithUndo(element, replacement, innerStart, innerEnd)) {
      return true;
    }
  ...
  function applyInlineStyleInTextarea(element, wrapper) {
    ...
    if (start !== end && tryApplyInlineStyleInTextareaContext(element, wrapper, start, end, selected)) {
      return true;
    }
    ...
    if (insertIntoTextareaWithUndo(element, replacement, start, start + replacement.length)) {
      return true;
    }
  ...
  function htmlLooksLikeBlock(html) {
    ...
    return /^<(address|article|aside|blockquote|div|dl|fieldset|figcaption|figure|footer|form|h[1-6]|header|hr|li|main|nav|ol|p|pre|section|table|ul|details)\b/i.test(trimmed);
  }
  ...
  function normalizeInnerHtml(selectedHtml, selectedText, markerId) {
    ...
    if (markerId) {
      return '<p><span id="' + markerId + '" data-selsup-caret="1">\uFEFF</span></p>';
    }
  ...
  function buildTinyMceHtml(wrapper, innerHtml) {
    ...
    if (wrapper.kind === "note" || wrapper.kind === "step") {
      return '<div class="' + wrapper.className + '">' + getTitleHtml(wrapper) + innerHtml + "</div>";
    }
  ...
  function wrapInTinyMce(wrapper) {
    ...
    const markerId = "selsup-caret-" + Date.now() + "-" + Math.floor(Math.random() * 100000);

    debugLog("wrap tinymce", wrapper.label);

    editor.undoManager.transact(function () {
      if (wrapper.insertOnly) {
        editor.execCommand("mceInsertContent", false, wrapper.htmlContent || "");
        return;
      }

      const selectedHtml = editor.selection.getContent({ format: "html" });
      const selectedText = editor.selection.getContent({ format: "text" });
      const innerHtml = normalizeInnerHtml(selectedHtml, selectedText, markerId);
  ...
  function wrapInDocumentSelection(doc, wrapper) {
    ...
    const innerHtml = wrapper.insertOnly ? "" : normalizeInnerHtml(selectedHtml, selectedText, "");
    const html = wrapper.insertOnly ? wrapper.htmlContent || "" : buildTinyMceHtml(wrapper, innerHtml);
  ...
  function getContentSourceElement(element) {
    const innerContainer = findDirectChildByClass(element, "wp-block-group__inner-container");
  ...
  function unwrapSelSupBlockElement(element) {
    ...
    if (element.classList.contains("ss-note") && unwrapLegacyBodyIfNeeded(element, "ss-note-body")) return;
    if (element.classList.contains("ss-step") && unwrapLegacyBodyIfNeeded(element, "ss-step-body")) return;
  ...
  function unwrapInlineFormatting(root) {
    ...
    const elements = Array.from(root.querySelectorAll(inlineFormattingSelector)).reverse();
  ...
  function cleanFormattingDom(root) {
    ...
    const blockWrappers = Array.from(
      root.querySelectorAll(".ss-note, .ss-step, .ss-faq, .code-block-wrapper, .temporary-hidden"),
    ).reverse();
  ...
  function htmlSelectionContainsClearableMarkup(html) {
    return (
      /<(span|font|strong|b|em|i|u|s|strike|mark|small|sub|sup|code)\b/i.test(html) ||
      /class=["'][^"']*(ss-note|ss-step|ss-faq|code-block-wrapper|temporary-hidden|ss-text-|selsup-text-)/i.test(html) ||
  ...
  function clearFormattingInTextarea(element) {
    ...
    if (hasSelection && !htmlSelectionContainsClearableMarkup(target)) {
      const expandedRange = findEnclosingInlineFormattingRange(value, start, end);
  ...
  function registerGutenbergInlineFormats() {
    ...
    Object.keys(inlineColorFormatMap).forEach(function (colorKey) {
      ...
      try {
        if (richText.getFormatType && richText.getFormatType(formatName)) return;
        ...
        debugLog("registered Gutenberg format", formatName, inlineColorClassMap[colorKey]);
      } catch (error) {
    ...
    Object.keys(inlineWeightFormatMap).forEach(function (weightKey) {
      ...
      try {
        ...
        debugLog("registered Gutenberg format", formatName, inlineWeightClassMap[weightKey]);
      } catch (error) {
        debugWarn("failed to register Gutenberg format", formatName, error);
      }
    });
  ...
  function getGutenbergInlineFormatName(wrapper) {
    if (wrapper.kind === "inline-color") return inlineColorFormatMap[wrapper.colorKey];
    if (wrapper.kind === "inline-weight") return inlineWeightFormatMap[wrapper.weightKey];
  ...
  function removeConflictingGutenbergInlineFormats(value, wrapper, startOffset, endOffset) {
    let nextValue = value;

    getConflictingGutenbergInlineFormats(wrapper).forEach(function (formatName) {
      try {
        nextValue = window.wp.richText.removeFormat(nextValue, formatName, startOffset, endOffset);
      } catch (error) {
        debugWarn("failed to remove conflicting Gutenberg format", formatName, error);
      }
    });
  ...
  function createRichTextValueFromAttribute(value) {
    ...
    if (html === null) {
      debugWarn("cannot convert Gutenberg attribute to HTML", describeValue(value));
      return null;
    }
    ...
    try {
      ...
      debugWarn("richText.create({ html }) failed", error, { html: html.slice(0, 180) });
    }
  ...
  function updateGutenbergRichTextSelection(transformer, options) {
    ...
    const allowStoredSelection = !options || options.allowStoredSelection !== false;
    const liveInfo = getGutenbergSelectionInfo(select);
    const storedInfo = allowStoredSelection ? getStoredGutenbergSelectionInfo(select) : null;
  ...
  function getGutenbergIdsBetweenSelection(select) {
    ...
    return ids.slice(Math.min(startIndex, endIndex), Math.max(startIndex, endIndex) + 1);
  }
  ...
  function hasLiveGutenbergDomTextSelection() {
    ...
    if (isNodeInsideToolbar(selection.anchorNode) || isNodeInsideToolbar(selection.focusNode)) return false;

    return isNodeInsideGutenbergEditor(selection.anchorNode) || isNodeInsideGutenbergEditor(selection.focusNode);
  }
  ...
  function preserveGutenbergSelectionBeforeToolbar() {
    ...
    toolbarSelectionLockUntil = Date.now() + GUTENBERG_TOOLBAR_SELECTION_LOCK_MS;
    ...
    if (ids.length) {
      ...
      debugLog("remember Gutenberg blocks before toolbar", lastGutenbergSelectedBlockIds);
    }
  }
  ...
  function rememberGutenbergSelection() {
    ...
    if (info && (hasLiveText || toolbarLocked)) {
      storeGutenbergTextSelectionInfo(info, hasLiveText ? "live-text" : "toolbar-locked-store");
  ...
  function protectGutenbergSelection(element) {
    ...
    ["pointerdown", "mousedown", "touchstart"].forEach(function (eventName) {
      element.addEventListener(eventName, preserveGutenbergSelectionBeforeToolbar, true);
    });
  }
  ...
  function applyInlineStyleInGutenberg(wrapper) {
    ...
    const info = getGutenbergSelectionInfo(select) || getStoredGutenbergSelectionInfo(select);
    ...
    if (info) {
      return updateGutenbergRichTextSelection(function (value, startOffset, endOffset) {
    ...
    if (!ids.length) {
      debugWarn("Gutenberg inline style failed: no text selection and no block ids");
      return false;
    }
    ...
    debugLog("apply inline Gutenberg fallback block result", { ids: ids, changed: changed });
  ...
  function isSelSupWrapperBlock(block) {
    return blockHasAnyClass(block, ["ss-note", "ss-step", "ss-faq", "code-block-wrapper", "temporary-hidden"]);
  }

  function isSelSupBodyBlock(block) {
    return blockHasAnyClass(block, ["ss-note-body", "ss-step-body", "ss-faq-body"]);
  }
  ...
  function clearFormattingInGutenberg() {
    ...
    if ((hasLiveText || storedTextSelection) && clearGutenbergRichTextSelectionFormatting({ allowStoredSelection: true })) {
    ...
    if (!selectedIds.length) {
      debugWarn("clear Gutenberg failed: no live text selection and no selected blocks");
      return false;
    }
    ...
    selectedIds.forEach(function (selectedId) {
      ...
      if (wrapperId && !handledWrapperIds.has(wrapperId)) {
        ...
        if (unwrappedBlocks) {
          dispatch.replaceBlocks([wrapperId], unwrappedBlocks.length ? unwrappedBlocks : [makeParagraphBlock()]);
    ...
    debugLog("clear Gutenberg result", { selectedIds: selectedIds, changed: changed });
  ...
  function makeGutenbergBlock(wrapper, innerBlocks) {
    ...
    if (wrapper.kind === "faq") {
      const contentBlocks =
        innerBlocks && innerBlocks.length
          ? innerBlocks
          : [makeParagraphBlock("Здесь можно разместить ответ, который будет скрыт до раскрытия блока.")];
    ...
    if (wrapper.kind === "note" || wrapper.kind === "step") {
      const titleBlock = makeParagraphBlock(
        '<strong style="' + wrapper.titleStyle + '">' + escapeHtml(wrapper.title) + "</strong>",
      );

      const contentBlocks = innerBlocks && innerBlocks.length ? innerBlocks : [makeParagraphBlock()];

      return makeGroupBlock(wrapper.className, [titleBlock].concat(contentBlocks));
    }

    return makeGroupBlock(wrapper.className, innerBlocks && innerBlocks.length ? innerBlocks : [makeParagraphBlock()]);
  }

  function insertGutenbergAfterSelection(block, select, dispatch, selectedIds) {
    const lastSelectedId = selectedIds.length ? selectedIds[selectedIds.length - 1] : null;
    const rootClientId = lastSelectedId ? select.getBlockRootClientId?.(lastSelectedId) : undefined;
    const selectedIndex = lastSelectedId ? select.getBlockIndex?.(lastSelectedId, rootClientId) : undefined;
    const insertIndex = Number.isInteger(selectedIndex) ? selectedIndex + 1 : undefined;
  ...
  function applyGutenbergWrapper(wrapper) {
    ...
    debugLog("apply Gutenberg wrapper", { wrapper: wrapper.label, selectedIds: selectedIds });

    if (wrapper.kind === "separator" || wrapper.kind === "html") {
      const block = makeGutenbergBlock(wrapper, []);
      return insertGutenbergAfterSelection(block, select, dispatch, selectedIds);
    }
  ...
  function flattenLegacyWrapperHtml(element, bodyClassName) {
    const doc = document.implementation.createHTMLDocument("");
    const container = doc.createElement("div");
    const body = element.querySelector(":scope > ." + bodyClassName) || element.querySelector("." + bodyClassName);
  ...
  function htmlElementToRepairedBlocks(element) {
    ...
    if (element.classList.contains("code-block-wrapper")) {
      return [makeGroupBlock("code-block-wrapper", rawHtmlToBlocks(element.innerHTML))];
    }

    if (element.classList.contains("temporary-hidden")) {
      return [makeGroupBlock("temporary-hidden", rawHtmlToBlocks(element.innerHTML))];
    }

    if (element.matches("details.ss-faq")) {
      const summary = element.querySelector(":scope > summary");
      const summaryText = summary ? summary.textContent.trim() || "Частый вопрос" : "Частый вопрос";
      const bodyContainer = document.implementation.createHTMLDocument("").createElement("div");
      ...
      try {
        ...
        return [
          makeGroupBlock(
            "ss-faq",
            [makeParagraphBlock("<strong>" + escapeHtml(summaryText) + "</strong>")].concat(
              innerBlocks.length ? innerBlocks : [makeParagraphBlock()],
            ),
          ),
        ];
  ...
  function repairSelectedGutenbergHtmlBlocks() {
    ...
    selectedIds.forEach(function (id) {
      ...
      if (!/ss-note|ss-step|ss-faq|code-block-wrapper|temporary-hidden/.test(content)) return;
    ...
    debugLog("repair selected Gutenberg HTML blocks", { selectedIds: selectedIds, repaired: repaired });
  ...
  function getGutenbergHeaderTarget() {
    const leftTools =
      document.querySelector(".editor-header__toolbar .editor-document-tools__left") ||
      document.querySelector(".edit-post-header-toolbar .editor-document-tools__left") ||
    ...
    const documentToolbar =
      document.querySelector(".editor-header__toolbar .editor-document-tools") ||
      document.querySelector(".edit-post-header-toolbar .editor-document-tools");
  ...
  function getToolbarTarget() {
    ...
    if (classicTarget && !document.body.classList.contains("block-editor-page")) {
      return {
  ...
  function buildControls(container, buttonClass, compact) {
    ...
    function row(label, groupKey) {
      const group = compact ? document.createElement("div") : createToolbarGroup();
    ...
    const blockSelect = createSelect("selsup-wrapper-block-select", blockWrappers);
    const textColorSelect = createSelect("selsup-wrapper-color-select", textColorWrappers);
    const textWeightSelect = createSelect("selsup-wrapper-weight-select", textWeightWrappers);
    ...
    const wrapButton = makeButton("Wrap", "Apply selected block wrapper", buttonClass, function () {
      wrapSelection(blockWrappers[Number(blockSelect.value)]);
    });

    const quickInfoButton = makeButton("Info", "Blue info block", buttonClass, function () {
      wrapSelection(findWrapper("Blue info block"));
    });

    const quickWarnButton = makeButton("Warn", "Orange warning block", buttonClass, function () {
      wrapSelection(findWrapper("Orange warning block"));
    });

    const softLineButton = makeButton("Soft", "Soft horizontal line", buttonClass, function () {
      wrapSelection(findWrapper("Blue-dot separator"));
    });

    const sectionLineButton = makeButton("Section", "Section horizontal line", buttonClass, function () {
      wrapSelection(findWrapper("Section separator"));
    });

    const applyColorButton = makeButton("Apply color", "Apply selected text color", buttonClass, function () {
      wrapSelection(textColorWrappers[Number(textColorSelect.value)]);
    });

    const applyWeightButton = makeButton("Apply weight", "Apply selected text weight", buttonClass, function () {
      wrapSelection(textWeightWrappers[Number(textWeightSelect.value)]);
    });

    const clearButton = makeButton("Clear", "Clear formatting from selection/current block", buttonClass, function () {
      wrapSelection(findWrapper("Clear formatting"));
    });

    const repairButton = makeButton("Repair HTML", "Convert old SelSup HTML blocks into Gutenberg blocks", buttonClass, function () {
      repairLegacyHtml();
    });
  ...
  function createGutenbergMorePanel(buttonClass) {
    const panel = createPanelBase("selsup-wrapper-more-panel", "More SelSup tools");
  ...
  function hideGutenbergToolPanels(exceptPanel) {
    ["selsup-wrapper-panel", "selsup-wrapper-more-panel"].forEach(function (id) {
  ...
  function getGutenbergRightBoundary(toolbar, toolbarLeft) {
    ...
    const leftEdge = Number.isFinite(toolbarLeft) ? toolbarLeft : toolbar.getBoundingClientRect().left;
    ...
    [
      ...
      document.querySelectorAll(selector).forEach(function (element) {
        if (!element || toolbar.contains(element) || element.contains(toolbar)) return;
  ...
  function getAdaptiveToolbarAvailableWidth(toolbar) {
    ...
    const rightBoundary = getGutenbergRightBoundary(toolbar, toolbarLeft);
  ...
  function getPanelRowsByGroup(panel) {
    ...
    return Array.from(panel.querySelectorAll("[data-selsup-group]")).filter(function (row) {
      return row.closest("#selsup-wrapper-panel, #selsup-wrapper-more-panel") === panel;
    });
  }
  ...
  function applyAdaptiveGutenbergToolbarLayout(toolbar) {
    ...
    const inlineControls = toolbar.querySelector(".selsup-gutenberg-inline-controls");
    const moreControls = toolbar.querySelector(".selsup-gutenberg-more-controls");
    const collapsedControls = toolbar.querySelector(".selsup-gutenberg-collapsed-controls");
    const morePanel = document.getElementById("selsup-wrapper-more-panel");

    if (!inlineControls || !moreControls || !collapsedControls) return;

    const groups = Array.from(inlineControls.querySelectorAll("[data-selsup-group]"));
    ...
    function finish(mode) {
      ...
      const signature = [mode, availableWidth, neededWidth, hiddenGroups.join("|")].join("::");
      ...
      if (signature !== selsupLastAdaptiveLayoutSignature) {
        ...
        debugLog("gutenberg adaptive toolbar layout", {
          mode: mode,
    ...
    for (const groupKey of GUTENBERG_COLLAPSE_ORDER) {
      const group = inlineControls.querySelector('[data-selsup-group="' + groupKey + '"]');
  ...
  function installDevicePixelRatioWatcher(toolbar) {
    function refreshForPixelRatioChange(reason) {
      ...
      if (currentRatio === selsupLastKnownDevicePixelRatio && reason !== "forced") return;
    ...
    try {
      if (selsupDevicePixelRatioMediaQuery) {
        if (typeof selsupDevicePixelRatioMediaQuery.removeEventListener === "function") {
          selsupDevicePixelRatioMediaQuery.removeEventListener("change", refreshForPixelRatioChange);
        } else if (typeof selsupDevicePixelRatioMediaQuery.removeListener === "function") {
          selsupDevicePixelRatioMediaQuery.removeListener(refreshForPixelRatioChange);
        }
      }
    } catch (error) {}
    ...
    try {
      ...
      if (typeof selsupDevicePixelRatioMediaQuery.addEventListener === "function") {
        selsupDevicePixelRatioMediaQuery.addEventListener("change", refreshForPixelRatioChange);
      } else if (typeof selsupDevicePixelRatioMediaQuery.addListener === "function") {
        selsupDevicePixelRatioMediaQuery.addListener(refreshForPixelRatioChange);
      }
    } catch (error) {
  ...
  function installAdaptiveGutenbergToolbarResize(toolbar) {
    ...
    if (window.ResizeObserver) {
      try {
        ...
        const header = getClosestGutenbergHeaderElement(toolbar) || toolbar.parentElement;

        if (header) observer.observe(header);
        // CHANGE 3.9.4: parentElement не наблюдаем, потому что его ширина может меняться от самого toolbar.

        // CHANGE 3.9.3: body/documentElement не наблюдаем.
        // Их размер меняют редактор, Yoast и метабоксы, из-за чего layout пересчитывался примерно каждую секунду.
        [
          ".editor-header__settings",
          ".edit-post-header__settings",
          ".interface-pinned-items",
  ...
  function buildAdaptiveGutenbergToolbar(toolbar, buttonClass) {
    toolbar.classList.add("selsup-gutenberg-adaptive-toolbar");
    ...
    const moreButton = makeButton("More ↓", "Open hidden SelSup tools", buttonClass, function () {
    ...
    const toggleButton = makeButton("SelSup ▾", "Open SelSup tools", buttonClass, function () {
  ...
  function createInlineToolbar() {
    ...
    const buttonClass = toolbarTarget.mode === "classic" ? "button" : "components-button is-secondary is-compact";
    ...
    if (toolbarTarget.mode === "gutenberg-header") {
      ...
      buildControls(toolbar, buttonClass, false);
    }
  ...
  function createLoadedBadge() {
    ...
    badge.textContent = SELSUP_DEBUG ? "SelSup wrappers loaded, debug on" : "SelSup wrappers loaded";
  ...
  function init() {
    ...
    document.addEventListener("selectionchange", scheduleRememberGutenbergSelection, true);
    document.addEventListener("mouseup", scheduleRememberGutenbergSelection, true);
    document.addEventListener("keyup", scheduleRememberGutenbergSelection, true);
    document.addEventListener("focusin", scheduleRememberGutenbergSelection, true);
